...
### Hallazgos clave del análisis multivariado:
1. **Compensación de aglutinantes:** Existen correlaciones negativas significativas entre el cemento y sus sustitutos: cemento vs ceniza ($r = -0.397$) y cemento vs escoria ($r = -0.275$). Esto refleja que en el diseño de mezclas, al incorporar ceniza volante o escoria se reduce proporcionalmente el cemento.
2. **Efecto reductor del superplastificante:** Se observa una fuerte correlación negativa entre el agua y el superplastificante ($r = -0.657$), confirmando la eficacia de este aditivo químico para disminuir la demanda de agua.
3. **Comportamiento no lineal del curado:** La relación entre la edad y la resistencia exhibe una desaceleración con el tiempo, sugiriendo una transformación funcional para capturar mejor su impacto.
...
### Correlación de las nuevas variables con la Resistencia

Comparamos la fuerza de asociación lineal de las nuevas variables con la variable respuesta:

```{r tabla-correlaciones-fe}
df_fe %>%
  select(resistencia, cemento, total_cementante, agua, razon_agua_cementante, edad, log_edad, pct_ceniza) %>%
  cor() %>%
  round(3) %>%
  as.data.frame() %>%
  select(Resistencia = resistencia)
```

Como se aprecia, la **razón agua/cementante** ($r = -0.623$) y el **total de cementante** ($r = +0.613$) se consolidan como los predictores más potentes de la resistencia, mientras que $\log(\text{edad})$ ($r = +0.552$) supera considerablemente a la edad en escala lineal ($r = +0.329$).
